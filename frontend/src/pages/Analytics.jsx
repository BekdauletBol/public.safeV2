...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ ...mono, ...card, fontSize: 11, padding: '10px 14px' }}>
      <p style={{ color: 'rgba(255,255,255,0.4)', marginBottom: 6, letterSpacing: '0.04em' }}>{label}</p>
      {payload.map((p) => (
...
const ghostBtn = {
  ...
  border: '1px solid rgba(255,255,255,0.1)',
  borderRadius: '3px', color: 'rgba(255,255,255,0.4)',
        ...
        <div ref={chartRef} className="flex flex-col gap-5">
          ...
          <div style={card}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span style={{ fontSize: 11, letterSpacing: '0.08em', color: 'rgba(255,255,255,0.5)' }}>hourly traffic</span>
            </div>
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={hourlyData} margin={{ left: -10, right: 10 }}>
                ...
                <XAxis dataKey="time" tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10, fontFamily: 'JetBrains Mono' }} tickLine={false} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={false} />
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', fontFamily: 'JetBrains Mono' }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div style={card}>
              <div className="flex items-center gap-2 mb-4">
                <BarChart2 size={13} style={{ color: '#ffb830' }} />
                <span style={{ fontSize: 11, letterSpacing: '0.08em', color: 'rgba(255,255,255,0.5)' }}>daily totals (7 days)</span>
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={dailyData} margin={{ left: -10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                  <XAxis dataKey="date" tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10, fontFamily: 'JetBrains Mono' }} tickLine={false} />
                  <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="total" name="total" fill="#ffb830" radius={[2, 2, 0, 0]} opacity={0.8} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={card}>
              <div className="flex items-center gap-2 mb-4">
                <Clock size={13} style={{ color: '#ff3d6b' }} />
                <span style={{ fontSize: 11, letterSpacing: '0.08em', color: 'rgba(255,255,255,0.5)' }}>peak hour distribution</span>
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={peaks.slice(0, 12)} margin={{ left: -10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                  <XAxis dataKey="hour" tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10, fontFamily: 'JetBrains Mono' }} tickLine={false} />
                  <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="avg" name="avg people" fill="#ff3d6b" radius={[2, 2, 0, 0]} opacity={0.8} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
